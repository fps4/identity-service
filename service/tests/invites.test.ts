...
function makeState(tenantOverrides: Record<string, unknown> = {}) {
  return {
    Tenant: fakeCollection([{
      _id: 't1', name: 'Acme', status: 'active',
      oauth: { enabled: true, allowedGrantTypes: ['password'], allowedRoles: ['tenant_admin', 'member'], idp: { provider: 'local' }, ...tenantOverrides }
    }]),
    User: fakeCollection([]),
...
describe('admin service — invites (RQ-0013)', () => {
  ...
  beforeEach(() => { state = makeState(); admin = createAdminService(deps(state)); });

  it('creates an invite, returns the code once, and stores only its digest', async () => {
    const { inviteId, code, expiresAt } = await admin.createInvite({ tenantId: 't1' });
  ...
  it('normalizes a bound email and validates roles against the tenant vocabulary', async () => {
    const { inviteId } = await admin.createInvite({ tenantId: 't1', email: ' New@Example.COM ', roles: ['member'] });
    expect(state.Invite._items.find((i) => i._id === inviteId)).toMatchObject({ email: 'new@example.com', roles: ['member'] });
    await expect(admin.createInvite({ tenantId: 't1', roles: ['superuser'] }))
      .rejects.toMatchObject({ status: 400, code: 'invalid_input' });
    await expect(admin.createInvite({ tenantId: 't1', email: 'not-an-email' }))
      .rejects.toMatchObject({ status: 400, code: 'invalid_email' });
    await expect(admin.createInvite({ tenantId: 'ghost' }))
      .rejects.toMatchObject({ status: 404, code: 'tenant_not_found' });
    await expect(admin.createInvite({ tenantId: 't1', maxUses: 0 }))
      .rejects.toMatchObject({ status: 400, code: 'invalid_input' });
  });

  it('lists invites with derived status and usedCount, never the code digest', async () => {
    const { inviteId } = await admin.createInvite({ tenantId: 't1', maxUses: 2, note: 'March cohort' });
    await admin.createInvite({ tenantId: 't1', expiresInHours: -1 }).catch(() => {}); // rejected, not stored
    state.Invite._items.find((i) => i._id === inviteId).usesRemaining = 1; // one redemption happened

    const listed = await admin.listInvites('t1');
  ...
  it('revokes an invite (listing it as revoked) and 404s on an unknown id', async () => {
    const { inviteId } = await admin.createInvite({ tenantId: 't1' });
    expect(await admin.revokeInvite(inviteId)).toEqual({ inviteId, revoked: true });
    expect((await admin.listInvites('t1'))[0].status).toBe('revoked');
...
describe('registration policy gate + redemption (RQ-0013)', () => {
  ...
  const setup = (registration?: 'open' | 'invite' | 'closed') => {
    state = makeState(registration ? { registration } : {});
    admin = createAdminService(deps(state));
    users = createUserService(deps(state));
  };
  const register = (email: string, inviteCode?: string) =>
    users.registerUser({ tenantId: 't1', email, password: 'long-enough-pw', inviteCode });

  it('an open tenant (or one with no policy) registers exactly as before, code or not', async () => {
    setup();
  ...
  it('a closed tenant refuses self-registration outright', async () => {
    setup('closed');
    await expect(register('a@x.test')).rejects.toMatchObject({ status: 403, code: 'registration_closed' });
  });

  it('an invite tenant requires a code, and rejects garbage/expired/revoked codes generically', async () => {
    setup('invite');
    ...
    const expired = await admin.createInvite({ tenantId: 't1', expiresInHours: 1 });
    ...
    const revoked = await admin.createInvite({ tenantId: 't1' });
  ...
  it('redeems a valid code: user created, roles stamped, a use consumed, redemption audited', async () => {
    setup('invite');
    const { inviteId, code } = await admin.createInvite({ tenantId: 't1', roles: ['member'] });
    ...
    expect(state.AuditLog._items[0]).toMatchObject({
      ...
      meta: { tenantId: 't1', userId: user.id, email: 'new@x.test' }
    });
  ...
  it('a multi-use cohort code admits exactly maxUses people', async () => {
    setup('invite');
    const { code } = await admin.createInvite({ tenantId: 't1', maxUses: 2 });
  ...
  it('an email-bound invite only admits (and then vouches) its address; a mismatch refunds the use', async () => {
    setup('invite');
    const { code } = await admin.createInvite({ tenantId: 't1', email: 'invited@x.test' });
  ...
  it('email_taken after a valid code refunds the use (a rejected registration never burns one)', async () => {
    setup('invite');
    state.User._items.push({ _id: 'u0', tenantId: 't1', email: 'taken@x.test', createdAt: new Date(0) });
    const { code } = await admin.createInvite({ tenantId: 't1' });
  ...
  it('input validation fires before any invite is consulted (no use burned on a weak password)', async () => {
    setup('invite');
    const { code } = await admin.createInvite({ tenantId: 't1' });
    await expect(users.registerUser({ tenantId: 't1', email: 'a@x.test', password: 'short', inviteCode: code }))
