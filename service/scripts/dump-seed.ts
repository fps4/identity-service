 ...
 * APPLICATIONS currently in the table (each with its role catalogue, audience, and credentials) into a
...
/** Minimal shapes the builder needs from the documents (kept loose so tests need no table). */
...
async function main() {
  ...
  const applications = (await store.applications.list() as DumpApplication[]).sort((a, b) => a._id.localeCompare(b._id));
  const credentials = (await store.clients.list() as DumpCredential[]).sort((a, b) => a._id.localeCompare(b._id));
  const users = flags.includeUsers ? (await store.users.list() as DumpUser[]).sort((a, b) => a.email.localeCompare(b.email)) : [];
  const assignments: DumpAssignment[] = [];
  if (flags.includeUsers) for (const u of users) assignments.push(...await store.assignments.listByUser(u._id));
