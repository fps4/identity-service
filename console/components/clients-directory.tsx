...
// The applications (OAuth clients) directory (RQ-0017, ADR-0014): client-side search + a table with
// type badges, opening a per-application detail drawer. Reuses the listClients read and the client
// server actions (register / rotate / delete).
...
export function ClientsDirectory({ clients, loadError }: {
  clients: Client[];
  loadError?: string;
}) {
  ...
  return (
    <div className="space-y-4">
              ...
              <tr>
                <td colSpan={5}>
                  <div className="py-12 text-center text-sm text-muted-foreground">
                    {clients.length ? 'No applications match your search.' : 'No applications yet.'}
      ...
      {selected && (
        <ClientDetailDrawer client={selected} onClose={() => setSelectedId(null)} />
      )}
    ...
    <div role="dialog" aria-modal="true" aria-label="Register application" className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md space-y-4 rounded-lg border bg-background p-6 shadow-lg">
        <div>
          ...
          <p className="text-sm text-muted-foreground">A new OAuth client. A confidential client’s secret is shown once.</p>
        </div>
