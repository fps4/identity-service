...
// user drawer's assignments section (ADR-0020) hydrates via fetchUserAssignments on mount, so it must
// resolve to an array.
vi.mock('@/app/actions', () => ({
  ...
  fetchUserAssignments: vi.fn().mockResolvedValue([]),
  assignUser: vi.fn(), updateAssignment: vi.fn(), revokeAssignment: vi.fn(),
}));
vi.mock('sonner', () => ({ toast: { success: vi.fn(), error: vi.fn() } }));

// ADR-0020: users carry no deployment-wide roles anymore (access is per-application via assignments).
